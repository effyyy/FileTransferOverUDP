...
int main(int argc, char * argv[]){
  ...
  while (1)
  {
    n = recvfrom(s, buf, sizeof(buf), 0, (struct sockaddr*)&client_addr, addr_size);
    printf("Server got message: %s [%d bytes]\n", buf, len);
    ...
    /* send Acknowledgement To Server to server  
    fgets(buf, sizeof(buf), "ACK");
    buf[BUF_SIZE-1] = '\0';
    len = strlen(buf) + 1;
    if (sendto(s, buf, len, 0, (struct sockaddr *)&sin, sizeof(sin)) < 0) { 
      perror("Client: sendto()"); 
